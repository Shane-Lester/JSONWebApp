  ...
  .factory('Items', function () {
    ...
    service.move = function (index, state, direction) {
      ...
      if (state == 'department') {
        ...
        service.entries.department.splice(index + direction, 0, moveItem)
      } else {
    ...
    service.save = function (newObject, index) {
      if (index == service.entries.length) {
        service.entries.department.push(newObject)
      } else {
        // need to add the objects
        // service.entries.department[index] is the correct place
        service.entries.department[index] = newObject
      }
    }

    service.remove = function (index, state) {
      if (state == 'department') {
        service.entries.department.splice(index, 1)
      } else {
